...
//	printf("%s  ,%s\r\n",msgname,sig->name);
	if (sig->scaling != 1.0 || sig->offset != 0.0)
		type = "double";
	fprintf(o, "int encode_%s_%s_%s(can_obj_%s_t *o, %s in)", msgname, sig->name, god, god, copts->use_doubles_for_encoding ? "double" : type);
	if (header)
		return fputs(";\n", o);
	fputs(" {\n", o);
...
static int signal2scaling_decode(const char *msgname, unsigned id, signal_t *sig, FILE *o, bool header, const char *god, dbc2c_options_t *copts) 
{
	...
	if (sig->scaling != 1.0 || sig->offset != 0.0)
		type = "double";
	fprintf(o, "int decode_%s_%s_%s(const can_obj_%s_t *o, %s *out)", msgname,sig->name, god, god, copts->use_doubles_for_encoding ? "double" : type);
	if (header)
		return fputs(";\n", o);
	fputs(" {\n", o);
	if (copts->generate_asserts) {
